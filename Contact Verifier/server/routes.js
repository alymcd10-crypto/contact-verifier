...
import { rawLookup as pdlRaw } from './sources/pdl.js';
...
// Debug — returns raw PDL JSON. Use to diagnose missing email/address.
//   curl '.../api/debug/pdl?name=Jane%20Lee&company=...'
router.get('/debug/pdl', async (req, res) => {
  try {
    const out = await pdlRaw({
      name: req.query.name,
