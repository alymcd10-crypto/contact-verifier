...
export async function verifyContact(contact, opts = {}) {
  ...
  // Derive a website URL even if SerpAPI didn't find one — use the contact's existing
  // email domain (if it's not a free provider) so the scraper can still run.
  const FREE = /^(gmail|yahoo|hotmail|outlook|aol|icloud|me)\.com$/i;
  const emailDomain = contact.email?.includes('@') ? contact.email.split('@')[1] : null;
  const fallbackWebsite = (emailDomain && !FREE.test(emailDomain)) ? `https://${emailDomain}` : null;
  const websiteUrl  = serpRes?.websiteUrl || fallbackWebsite;
